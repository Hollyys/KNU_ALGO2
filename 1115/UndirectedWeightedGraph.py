...
def mstPrimEager(g):
    def include(v):
        ...
        for e in g.adj[v]:
            ...
            if not pq.contains(e.w): # pq가 포함 안하면
                pq.insert(e.w, e)
                print("insert", e.w, e)
            else:                 # pq가 포함 하면
                if pq.keyOf(e.w).weight > e.weight:
                    pq.decreaseKey(e.w, e)
    ...
    included = [False] * g.V  # included[v] == True if v is in the MST
    weightSum = 0  # Sum of edge weights in the MST
    pq = IndexMinPQ(g.V)  # Build a Index priority queue
    include(0)

    while not pq.isEmpty() and len(edgesInMST) < g.V - 1:
        e, w = pq.delMin() # return minKey, minIndex
        print("pop", e, w)
        if included[e.v] and included[e.w]: continue  # Ignore the edge v-w if both v and w are included in the MST
        edgesInMST.append(e)
        include(w)
