...
def sap(g, aList, bList):
    ...
    def putList(list):
        for i in list:
            if list == aList:
                q.put((i, 'a', 0))
                bfs_a.hasPathTo(i) == True
            else:
                q.put((i, 'b', 0))
                bfs_b.hasPathTo(i) == True
    ...
    while q.empty() == False:
        ...
        #if bfs_a.distTo(v) >= sap_lenth or bfs_b.distTo(v) >= sap_lenth:
        #    break

        for w in g.adj[v[0]]:
            if v[1] == 'a':
                if bfs_a.hasPathTo(w) == False:
                    bfs_a.visited[w] = True
                    q.put((w, 'a', bfs_a.distTo(w)))
                if bfs_b.hasPathTo(w) == True:
                    if bfs_a.distTo(w) + bfs_b.distTo(w) <= sap_lenth:
                        sap_lenth = bfs_a.distTo(w) + bfs_b.distTo(w)
            elif v[1] == 'b':
                if bfs_b.hasPathTo(w) == False:
                    bfs_b.visited[w] = True
                    q.put((w, 'b', bfs_b.distTo(w)))
                if bfs_a.hasPathTo(w) == True:
                    if bfs_a.distTo(w) + bfs_b.distTo(w) <= sap_lenth:
                        sap_lenth = bfs_a.distTo(w) + bfs_b.distTo(w)
    output = (v[0], sap_lenth)
    return output
