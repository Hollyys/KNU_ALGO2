...
def collinearpoints(point):
    ...
    def linereturn(start, x):
        ...
        for i in range(0, len(angle_logger)):
            if cnt_logger[i] >= 3:
                flag1 = angle_logger[i] in angle_max
                if flag1 == False:
                    cnt_max.append(cnt_logger[i])
                    point_max.append(point_logger[i])
                    angle_max.append(angle_logger[i])
                    start_max.append(start)
                else:
                    if cnt_max[angle_max.index(angle_logger[i])] <= cnt_logger[angle_logger.index(angle_logger[i])]:
                        start_max[angle_max.index(angle_logger[i])] = start
                        cnt_max[angle_max.index(angle_logger[i])] = cnt_logger[angle_logger.index(angle_logger[i])]
                        point_max[angle_max.index(angle_logger[i])] = point_logger[angle_logger.index(angle_logger[i])]
        # 중앙 data에 업로드

        for i in range(0, len(cnt_max)):
            insert = start_max[i] + point_max[i]
            flag3 = insert in collinearpoint
            if flag3 == False:
                collinearpoint.append(insert)

        return

    for i in range(0, len(point)):
        linereturn(point[i], i)
