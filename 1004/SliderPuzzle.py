...
def solveManhattan(initialBoard):
    ...
    cnt = 0
    route = [initialBoard] #경로저장 리스트
    board = initialBoard #현재 탐색할 보드
    prev_board = initialBoard

    while True:
        cnt = cnt + 1
        if board.isGoal() == True: #현재 커서가 목표점과 같으면 종료
            break
        ...
        for b in board.neighbors():
            ...
            if b not in route:
                k = b.manhattan()
                if min_manhattan == None or k <= min_manhattan:
                    min_manhattan = k
                    min_neighbor = b
                    flag = 1 # neighbors중에서 기존과 다른 경우의 수가 있음을 판별

        if flag != 1:
            break
        route.append(min_neighbor) #경로에 최소이동 neighbors추가
        board = min_neighbor
        flag = 0
...
if __name__ == "__main__":
    ...
    print('# Solvable in 0 move (already solved)')
    b10 = Board([[1,2,3],[4,5,6],[7,8,0]])    
    solveNprint(b10)
    
    # Solvable in 4 moves
    print('# Solvable in 4 moves')
    b11 = Board([[0,1,3],[4,2,5],[7,8,6]])
    solveNprint(b11)

    # Solvable in 2 moves
    print('# Solvable in 2 moves')
    b16 = Board([[0,1],[3,2]])
    solveNprint(b16)

    # Solvable in 14 moves
    print('# Solvable in 14 moves')
    b12 = Board([[8,1,3],[4,0,2],[7,6,5]])
    solveNprint(b12)

    # Solvable in 24 moves
    print('# Solvable in 24 moves')
